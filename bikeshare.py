...
def get_filters():
    ...
    print('you will get a new expermint')
    # TO DO: get user input for city (chicago, new york city, washington). HINT: Use a while loop to handle invalid inputs
    city_name = ''
    while city_name.lower() not in CITY_DATA:
        city_name = input("\nWhich city you want to check ? ( chicago, new york city, washington)\n")
    ...
    while month_name.lower() not in MONTH_DATA:
        month_name = input("\nwhich month you want to check? ('all' to apply no month filter or january, february, ... , june)\n")
        if month_name.lower() in MONTH_DATA:
            month = month_name.lower()
        else:
            print("Please, enter a valid name of month or all.\n")

    # TO DO: get user input for day of week (all, monday, tuesday, ... sunday)
    day_name = ''
    while day_name.lower() not in DAY_DATA:
        day_name = input("\nWhich day you want to check? ('all' to apply no day filter or sunday, monday, tuesday, ... saturday)\n")
        if day_name.lower() in DAY_DATA:
            day = day_name.lower()
        else:
            print("Please, enter a valid name of day or all.\n")
...
def user_stats(df, city):
    ...
    if city == 'chicago.csv' or city == 'new_york_city.csv':
        ...
        e_b = int(earliest_birth)
        m_r_b = int(most_recent_birth)
        m_c_b = int(most_common_birth)
    ...
    print("\nThis took %s seconds." % (time.time() - start_time))
    print('-'*40)
#this code used to display row data from the data file
def display_raw_data(df):
    """Displays raw data on user request. """
    print(df.head())
    next = 0
    while True:
        view_raw_data = input('\nWould you like to view next five rows of raw data? (yes or no.)\n')
...
def main():
    while True:
        ...
        user_stats(df, city)
        while True:
            view_raw_data = input('\nWould you like to view first five rows of raw data? (yes or no.)\n')
            if view_raw_data.lower() != 'yes':
                break
            display_raw_data(df)
            break
